...
class Program3
{
    ...
    public static void KayıtOluştur()
    {
        ...
        int IDCount = StudentCounter + 1;
        StudentID = YearOfEnrollment.Substring(6) + Faculty.Substring(0, 1).ToUpper() + Department.Substring(0, 1).ToUpper() + "00" + IDCount.ToString();

        student(FirstName, LastName, IDNumber, Gender, DateOfBirth, YearOfEnrollment, Faculty, Department, StudentID);
    ...
    public static void KayıtGörüntüle()
    {
        ...
        if (StudentCounter == 0)
        {
            System.Console.WriteLine(StudentCounter);
            Console.WriteLine("Henüz kayıtlı öğrenci yok!");
        }
        else
        {
            System.Console.WriteLine(StudentCounter);
            for (int i = 0; i < StudentCounter+2; i++)
            {
    ...
    public static void KayıtGüncelle()
{
    ...
    for (int i = 0; i < StudentCounter; i++)
    {
        ...
        if (studentDetails.Contains(SearchID))
        {
            Console.WriteLine("Öğrenci bilgileri: " + Students[i]);
            found = true;
            foundIndex = i;
    ...
    if (changeInfo == "6" || changeInfo == "7" || changeInfo == "8")
    {
        ...
        //string oldId = detaylar[8];

        string newStudentId = year + faculty.Substring(0, 1).ToUpper() + department.Substring(0, 1).ToUpper() + "00" + detaylar[8].Substring(8);
    ...
    public static void KayıtSil()
{
    ...
    public static void programBaşı()
    {
        ...
        for (int i = 0; i < lines.Length && i < Students.Length ; i++)
        {
            if (!string.IsNullOrWhiteSpace(lines[i]))
            {
                ...
                lineElements [^1] = lineElements[5].Substring(6) + lineElements[6].Substring(0, 1).ToUpper() + 
                                   lineElements[7].Substring(0, 1).ToUpper() + "00" + (i + 1).ToString();
    ...
    public static bool IDControl(string idNumber)
    {
        ...
        {   
            Console.Clear();
            System.Console.WriteLine("ID number 11 haneli olmalı!");
            return false;
        }
        ...
        for (int i = 0; i < idNumber.Length; i++)
        {
            ...
            {
                Console.Clear();
                System.Console.WriteLine("ID Number sadece Rakamlardan oluşmalı");
                return false;
            }
    ...
    public static bool TarihGecerliMi(string input, out string duzeltilmisTarih)
    {
        ...
        {
            Console.Clear();
            System.Console.WriteLine(input + " Tarihi doğru değil!");
            return false;
        }
